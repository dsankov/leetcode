...
class Solution:
    def maxWalls(self, robots: List[int], distance: List[int], walls: List[int]) -> int:
        ...
        dp = [SimpleNamespace(left=0, right=0) for _ in range(n)]
        ...
        for robot in range(1, n):
            ...
            effective_left_range = max(left_range, prev_right_range + 1)

            case_prev_shoot_right = (
                dp[robot - 1].right
                + count_walls(effective_left_range, robots[robot].pos)
            )
